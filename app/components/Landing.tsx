...
// Kube marketing landing (imported from Claude Design "Kube Landing").
// Markup + keyframes are the design verbatim; the scroll motion, the
// self-playing hero demo, and the auth/checkout overlay flows are a type-safe
// port of the design's logic, with the mock auth/checkout swapped for REAL
// Firebase auth + Stripe. Rendered at "/" for signed-out visitors.
...
type DemoAct = { status: string; caption: string; bar: number; count: string; pct: string; hold: number };
const DEMO_ACTS: DemoAct[] = [
  { status: "Reading", caption: "Reading your material…", bar: 0, count: "0 of 24 climbed", pct: "0%", hold: 4200 },
  { status: "Building", caption: "Laying out 6 units · 24 steps", bar: 14, count: "6 units found", pct: "14%", hold: 4600 },
  { status: "Ready", caption: "Your course, one clear path", bar: 38, count: "9 of 24 climbed", pct: "38%", hold: 4400 },
  { status: "Today", caption: "Today, let's do this — 12 minutes", bar: 38, count: "9 of 24 climbed", pct: "38%", hold: 5000 },
];
...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    const paintDemo = () => {
      ...
      if (!demo) return;
      const a = motionOn() ? act : 2;
      const cur = DEMO_ACTS[a];
      const set = (sel: string, txt: string) => {
        const el = demo.querySelector(sel) as HTMLElement | null;
        if (el) el.textContent = txt;
      };
      set("[data-demo-status]", cur.status);
      set("[data-demo-caption]", cur.caption);
      set("[data-demo-count]", cur.count);
      set("[data-demo-pct]", cur.pct);
      const bar = demo.querySelector("[data-demo-bar]") as HTMLElement | null;
      if (bar) bar.style.width = cur.bar + "%";

      qsa(demo, "[data-act]").forEach((el) => {
        const on = +(el.dataset.act || 0) === a;
        el.style.opacity = on ? "1" : "0";
        el.style.transform = on ? "translateY(0)" : "translateY(10px)";
        el.style.pointerEvents = on ? "auto" : "none";
        qsa(el, "[data-in]").forEach((it, i) => {
          const d = on ? 160 + i * 150 : 0;
          it.style.transitionDelay = d + "ms";
          it.style.opacity = on ? "1" : "0";
          it.style.transform = on ? "translateY(0) rotate(0deg) scale(1)" : it.dataset.in || "";
        });
      });

      qa("[data-demo-line]").forEach((el) => {
        const on = +(el.dataset.demoLine || 0) === a;
        el.style.opacity = on ? "1" : "0";
        el.style.transform = on ? "translateY(0)" : "translateY(10px)";
      });

      qsa(demo, "[data-dot]").forEach((el) => {
        const i = +(el.dataset.dot || 0);
        if (i === a) {
          el.style.transition = "none";
          el.style.width = "0%";
          void el.offsetWidth;
          el.style.transition = "width " + cur.hold + "ms linear";
          el.style.width = "100%";
        } else if (i < a) {
          el.style.transition = "width .3s ease";
          el.style.width = "100%";
          el.style.background = "#b4d8d5";
        } else {
          el.style.transition = "width .3s ease";
          el.style.width = "0%";
        }
        if (i === a) el.style.background = "#1f6f6b";
      });

      const daily = demo.querySelector("[data-demo-daily]") as HTMLElement | null;
      if (daily) daily.style.animation = a === 3 ? "k-locate 1.2s ease-out 1.1s 1" : "none";
    };

    const startDemo = () => {
      const demo = q("[data-demo]");
      if (!demo || demo.dataset.wired) return;
      demo.dataset.wired = "1";
      act = 0;
      paintDemo();
      if (!motionOn()) return;
      demoIO = new IntersectionObserver(
        (es) => es.forEach((en) => (demoVisible = en.isIntersecting)),
        { threshold: 0 }
      );
      demoIO.observe(demo);
      demoVisible = true;
      const step = () => {
        const hold = DEMO_ACTS[act].hold;
        demoTimer = setTimeout(() => {
          if (demoVisible !== false) {
            act = (act + 1) % DEMO_ACTS.length;
            paintDemo();
          }
          step();
        }, hold);
    ...
    qa("[data-click]").forEach((el) => {
      el.style.cursor = "pointer";
      el.addEventListener("click", (e) => {
        const fn = acts[el.dataset.click || ""];
        if (fn) fn(e, el);
      });
    });
  ...
  return (
    <>
      ...
      />
      <style dangerouslySetInnerHTML={{ __html: CSS + MOBILE_CSS }} />
      <div
        ref={rootRef}
